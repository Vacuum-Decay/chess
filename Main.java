...
public class Main {
	...
	boolean canBlackCastle = true;
	boolean canWhiteCastle = true;
...
//		if(!choseValidPiece)
		...
		if(turnTracker == blacksTurn && (destinationTileState == tileState.BLACK_BISHOP ||
		...
		switch(movedPiece) {
			...
			case tileState.WHITE_PAWN:
				...
				if(Math.abs(rank - rankDestination) == 2 && isValidMove) canEnPassant = true;
				break;
		...
		if(canEnPassant) {
//			enPassantTileCoordinate[0] = rankDestination;
//			enPassantTileCoordinate[1] = fileDestination;
		} else {
			enPassantTileCoordinate[0] = enPassantTileCoordinate[1] = -1;
		}
...
//			enPassantTileCoordinate[0] = startRow + direction;
			enPassantTileCoordinate[0] = (turnTracker == whitesTurn) ? 5 : 2;
			enPassantTileCoordinate[1] = sourceColumn;
		...
		if(Math.abs(columnDifference) == 1 && rowDifference == direction) {
			System.out.println(enPassantTileCoordinate[0] + ", " + enPassantTileCoordinate[1]);
			if(enPassantTileCoordinate[0] == destinationRow && enPassantTileCoordinate[1] == destinationColumn) {
				chessBoard[sourceRow][destinationColumn] = tileState.EMPTY;
			}
			return (destination != tileState.EMPTY &&
				   ...
				   || (enPassantTileCoordinate[0] == destinationRow && enPassantTileCoordinate[1] == destinationColumn);
		}
